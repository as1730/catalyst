...
export function AppSidebar({ isMobile = false }: { isMobile?: boolean }): JSX.Element {
  const location = useLocation();
  const content = (
    <div className={cn(
      "h-full flex flex-col items-center py-6",
      !isMobile && "fixed inset-y-0 left-0 w-20 bg-card border-r border-border z-50",
      isMobile && "w-full bg-background"
    )}>
      <div className="mb-10">
